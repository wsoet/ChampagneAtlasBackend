...
export async function museletProductsForProducer(
  producerName,
  { fetchImpl = fetch, now = Date.now(), locale = "en" } = {}
) {
  const query = searchName(producerName);
  if (!query) return [];

  const language = String(locale || "en").toLowerCase().split(/[-_]/)[0] === "nl" ? "nl" : "en";
  const cacheKey = `${language}:${query.toLocaleLowerCase("nl")}`;
  ...
  const products = (Array.isArray(body) ? body : []).flatMap((product) => {
    ...
    return [{
      ...
      sourceLanguage: "und",
      originalName: String(product.name || "").trim(),
      attribution: { provider:"Muselet.nl", sourceUrl:url }
    }];
  });
