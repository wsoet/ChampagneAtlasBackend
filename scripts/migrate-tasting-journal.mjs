...
const sql = await readFile(new URL("../migrations/014_tasting_journal.up.sql", import.meta.url), "utf8");
...
try {
  if (dryRun) {
    ...
    console.log("Tasting journal migration dry-run succeeded; transaction rolled back.");
  } else {
    await client.query(sql);
    console.log("Tasting journal migration applied.");
  }
} finally {
  await client.end();
}
