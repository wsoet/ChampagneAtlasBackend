...
const sql = await readFile(new URL("../migrations/010_shared_trip_groups.up.sql", import.meta.url), "utf8");
...
try {
  ...
  if (dryRun) {
    ...
    console.log("Shared trip-groups migration dry-run succeeded; transaction rolled back.");
  } else {
    await client.query("COMMIT");
    console.log("Shared trip-groups migration applied.");
  }
} catch (error) {
  await client.query("ROLLBACK");
  throw error;
