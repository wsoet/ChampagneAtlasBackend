...
const sql = await readFile(new URL("../migrations/022_web_analytics.up.sql", import.meta.url), "utf8");
...
try {
  if (dryRun) {
    ...
    console.log("Web analytics migration dry-run succeeded; transaction rolled back.");
  } else {
    await client.query(sql);
    console.log("Web analytics migration applied.");
  }
} finally {
  await client.end();
}
