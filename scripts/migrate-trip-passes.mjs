...
const sql = await readFile(new URL("../migrations/012_trip_passes.up.sql", import.meta.url), "utf8");
...
try {
  ...
  if (dryRun) {
    ...
    console.log("Trip Pass migration dry-run succeeded; transaction rolled back.");
  } else {
    await client.query("COMMIT");
    console.log("Trip Pass migration applied.");
  }
} catch (error) {
  await client.query("ROLLBACK");
  throw error;
